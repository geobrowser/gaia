...
pub fn validate_unix_timestamp(input: &str) -> Result<DateTime<Utc>, ValidationError> {
    ...
    DateTime::from_timestamp(timestamp, 0).ok_or(ValidationError::ParseFailure)
}
...
mod tests {
    ...
    fn test_validate_time() {
        ...
        assert_eq!(
            validate_time("invalid-date").err(),
            Some(ValidationError::ParseFailure)
        );
        assert_eq!(
            validate_time("2023-13-25").err(),
            Some(ValidationError::ParseFailure)
        ); // Invalid month
        assert_eq!(
            validate_time("2023-12-32").err(),
            Some(ValidationError::ParseFailure)
        ); // Invalid day
    }
    ...
    fn test_validate_unix_timestamp() {
        ...
        // Invalid cases
        assert_eq!(
            validate_unix_timestamp("").err(),
            Some(ValidationError::ParseFailure)
        );
        assert_eq!(
            validate_unix_timestamp("abc").err(),
            Some(ValidationError::ParseFailure)
        );
        assert_eq!(
            validate_unix_timestamp("12.34").err(),
            Some(ValidationError::ParseFailure)
        );
    }
    ...
    fn test_validate_time_comprehensive() {
        ...
        // Invalid cases
        assert_eq!(
            validate_time_comprehensive("").err(),
            Some(ValidationError::EmptyInput)
        );
        assert_eq!(
            validate_time_comprehensive("invalid-date").err(),
            Some(ValidationError::ParseFailure)
        );
        assert_eq!(
            validate_time_comprehensive("2023-13-25").err(),
            Some(ValidationError::ParseFailure)
        );
    }
    ...
    fn test_validate_time_string() {
        // Valid cases
        assert_eq!(
            validate_time_string("2023-12-25T10:30:00Z"),
            Ok("2023-12-25T10:30:00Z".to_string())
        );
        assert_eq!(
            validate_time_string("1703505000"),
            Ok("1703505000".to_string())
        );

        // Invalid cases
        assert_eq!(
            validate_time_string("").err(),
            Some(ValidationError::EmptyInput)
        );
        assert_eq!(
            validate_time_string("invalid-date").err(),
            Some(ValidationError::ParseFailure)
        );
    }
}
