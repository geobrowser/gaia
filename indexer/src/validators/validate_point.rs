...
mod tests {
    ...
    fn test_validate_point() {
        ...
        // Valid JSON cases
        assert_eq!(
            validate_point(r#"{"x":1.5,"y":2.5}"#),
            Ok(Point { x: 1.5, y: 2.5 })
        );
        assert_eq!(
            validate_point(r#"{"y":2.5,"x":1.5}"#),
            Ok(Point { x: 1.5, y: 2.5 })
        );
        ...
        assert_eq!(
            validate_point("1.5").err(),
            Some(ValidationError::ParseFailure)
        );
        assert_eq!(
            validate_point("1.5,2.5,3.5").err(),
            Some(ValidationError::ParseFailure)
        );
        assert_eq!(
            validate_point("abc,def").err(),
            Some(ValidationError::ParseFailure)
        );
        assert_eq!(
            validate_point("(1.5)").err(),
    ...
    fn test_validate_point_comprehensive() {
        // Valid cases
        assert_eq!(
            validate_point_comprehensive("1.5,2.5"),
            Ok(Point { x: 1.5, y: 2.5 })
        );
        assert_eq!(
            validate_point_comprehensive("(1.5,2.5)"),
        ...
        // Invalid cases
        assert_eq!(
            validate_point_comprehensive("").err(),
            Some(ValidationError::EmptyInput)
        );
        assert_eq!(
            validate_point_comprehensive("invalid").err(),
            Some(ValidationError::ParseFailure)
        );
    }
    ...
    fn test_validate_point_string() {
        ...
        assert_eq!(
            validate_point_string("(1.5,2.5)"),
            Ok("(1.5,2.5)".to_string())
        );

        // Invalid cases
        assert_eq!(
            validate_point_string("").err(),
            Some(ValidationError::EmptyInput)
        );
        assert_eq!(
            validate_point_string("invalid").err(),
            Some(ValidationError::ParseFailure)
        );
    }
    ...
    fn test_helper_functions() {
        ...
        assert_eq!(
            try_parse_json(r#"{"x":1.5,"y":2.5}"#),
            Ok(Point { x: 1.5, y: 2.5 })
        );
        assert_eq!(
            try_parse_json("invalid").err(),
            Some(ValidationError::ParseFailure)
        );
    }
}
