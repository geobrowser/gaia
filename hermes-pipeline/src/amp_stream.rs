...
pub async fn run_amp_stream(pipeline: &Pipeline) -> Result<(), PipelineError> {
    ...
    let mut resume_from: u64 = start_block;
    ...
    loop {
        ...
        let flight_client =
            FlightServiceClient::connect(url.clone()).await.map_err(anyhow::Error::from)?;
        ...
        while let Some(batch) = stream.next().await {
            ...
            let log_indexes = batch
                ...
                .ok_or_else(|| anyhow::anyhow!("missing log_index column"))?;
            let topic0 = batch
        ...
        if batch_count == 0 {
            warn!(
                resume_from,
                "Amp Flight stream ended without batches"
            );
        }
        warn!(
            resume_from,
