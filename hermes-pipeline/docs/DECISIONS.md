...
**Decision**: Add explicit `sequence` (uint32) and `is_last` (bool) fields to BlockchainMetadata. The sequence is derived from the action's index in the `Actions.actions` array from Amp, which is already in blockchain order (log index within the block). hermes-pipeline marks the final event with `is_last = true`. Consumers buffer events until `is_last` is received, then sort by sequence and process in blockchain order.
