...
describe("Property Resolver Tests", () => {
	...
	test("should get property by ID", async () => {
		const result = await Effect.runPromise(
			PropertyResolvers.getProperty(testPropertyId, createMockContext()).pipe(
				provideDeps,
			),
		);
	...
	test("should return null property for non-existent ID", async () => {
		...
		const result = await Effect.runPromise(
			PropertyResolvers.getProperty(nonExistentId, createMockContext()).pipe(
				provideDeps,
			),
		);
	...
	test("should handle different data types", async () => {
		...
		const result = await Effect.runPromise(
			PropertyResolvers.getProperty(numberPropertyId, createMockContext()).pipe(
				provideDeps,
			),
		);
		...
		await Effect.runPromise(
			Effect.gen(function* () {
				...
				yield* db.use(async (client) => {
					await client
						.delete(properties)
						.where(eq(properties.id, numberPropertyId));
				});
			}).pipe(provideDeps),
		);
	});
