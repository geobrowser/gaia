...
import {
	...
	getPropertyRenderableType,
} from "../kg/resolvers/properties";
...
describe("Types and Properties Integration Tests", () => {
	...
	describe("Types Query", () => {
		it("should return all types without spaceId filter", async () => {
			const result = await Effect.runPromise(
				provideDeps(getTypes({ limit: 10, offset: 0 })),
			);

			expect(result).toHaveLength(3);
			expect(result.map((r) => r.id).sort()).toEqual(
				[TYPE_ID_1, TYPE_ID_2, TYPE_ID_3].sort(),
			);
		});
		...
		it("should filter types by different spaceId", async () => {
			const result = await Effect.runPromise(
				provideDeps(
					getTypes({ limit: 10, offset: 0, spaceId: TEST_SPACE_ID_2 }),
				),
			);
		...
		it("should return empty array for non-existent spaceId", async () => {
			const result = await Effect.runPromise(
				provideDeps(getTypes({ limit: 10, offset: 0, spaceId: uuid() })),
			);
		...
		it("should respect limit parameter", async () => {
			const result = await Effect.runPromise(
				provideDeps(getTypes({ limit: 1, offset: 0 })),
			);
		...
		it("should respect offset parameter", async () => {
			...
			const offsetResults = await Effect.runPromise(
				provideDeps(getTypes({ limit: 10, offset: 1 })),
			);
		...
		it("should handle limit and offset together", async () => {
			const result = await Effect.runPromise(
				provideDeps(getTypes({ limit: 1, offset: 1 })),
			);
	...
	describe("Edge Cases", () => {
		it("should handle limit of 0", async () => {
			const result = await Effect.runPromise(
				provideDeps(getTypes({ limit: 0, offset: 0 })),
			);
		...
		it("should handle large offset", async () => {
			const result = await Effect.runPromise(
				provideDeps(getTypes({ limit: 10, offset: 1000 })),
			);
		...
		describe("Property Renderable Type", () => {
			...
			it("should return null for properties without renderable type relation", async () => {
				const result = await Effect.runPromise(
					provideDeps(
						getPropertyRenderableType(PROPERTY_ID_1, createMockContext()),
					),
				);
