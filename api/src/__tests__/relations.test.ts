		...
		entityBacklinksLoader: new DataLoader(async (keys: readonly any[]) => {
			...
			return Promise.all(
				keys.map(async (key) => {
					...
					const filteredBacklinks = Array.isArray(backlinks)
						? [...backlinks]
						: [];
...
describe("Relation Queries Integration Tests", () => {
	...
	describe("Single Relation Query", () => {
		it("should return a relation by ID", async () => {
			const result = await Effect.runPromise(
				getRelation(TEST_RELATION_1_ID).pipe(provideDeps),
			);
		...
		it("should return null for non-existent relation ID", async () => {
			...
			const result = await Effect.runPromise(
				getRelation(nonExistentId).pipe(provideDeps),
			);
		...
		it("should return relation with all expected fields", async () => {
			const result = await Effect.runPromise(
				getAllRelations({ toEntityId: TEST_ENTITY_2_ID }).pipe(provideDeps),
			);
	...
	describe("Multiple Relations Query", () => {
		it("should return all relations when no filter is provided", async () => {
			const result = await Effect.runPromise(
				getAllRelations({}).pipe(provideDeps),
			);
		...
		it("should filter relations by typeId", async () => {
			...
			expect(result.map((r) => r.id)).toEqual(
				expect.arrayContaining([TEST_RELATION_1_ID, TEST_RELATION_3_ID]),
			);
		...
		it("should filter relations by toEntityId", async () => {
			...
			expect(result).toHaveLength(2);
			expect(result.map((r) => r.id)).toEqual(
				expect.arrayContaining([TEST_RELATION_2_ID, TEST_RELATION_3_ID]),
			);
		...
		it("should filter relations by multiple criteria", async () => {
			...
			expect(
				result.every(
					(r) => r.typeId === TEST_TYPE_1_ID && r.fromId === TEST_ENTITY_1_ID,
				),
			).toBe(true);
		});
	...
	describe("Pagination", () => {
		it("should respect limit parameter", async () => {
			const result = await Effect.runPromise(
				getAllRelations({ limit: 2 }).pipe(provideDeps),
			);
		...
		it("should respect offset parameter", async () => {
			...
			const offsetResult = await Effect.runPromise(
				getAllRelations({ offset: 2 }).pipe(provideDeps),
			);

			expect(offsetResult).toHaveLength(2);
			expect(offsetResult.map((r) => r.id)).not.toEqual(
				allResults.slice(0, 2).map((r) => r.id),
			);
		});

		it("should combine limit and offset", async () => {
			const result = await Effect.runPromise(
				getAllRelations({ limit: 1, offset: 1 }).pipe(provideDeps),
			);
		...
		it("should handle offset beyond available results", async () => {
			const result = await Effect.runPromise(
				getAllRelations({ offset: 100 }).pipe(provideDeps),
			);
	...
	describe("Edge Cases", () => {
		it("should handle empty filter object", async () => {
			const result = await Effect.runPromise(
				getAllRelations({ filter: {} }).pipe(provideDeps),
			);
		...
		it("should handle null filter", async () => {
			const result = await Effect.runPromise(
				getAllRelations({ filter: null }).pipe(provideDeps),
			);
		...
		it("should handle undefined filter", async () => {
			const result = await Effect.runPromise(
				getAllRelations({ filter: undefined }).pipe(provideDeps),
			);
		...
		it("should handle zero limit", async () => {
			const result = await Effect.runPromise(
				getAllRelations({ limit: 0 }).pipe(provideDeps),
			);
		...
		it("should handle negative limit", async () => {
			const result = await Effect.runPromise(
				getAllRelations({ limit: -1 }).pipe(provideDeps),
			);
		...
		it("should handle negative offset", async () => {
			...
			await expect(
				Effect.runPromise(getAllRelations({ offset: -1 }).pipe(provideDeps)),
			).rejects.toThrow();
		});
	...
	describe("Verified Relations", () => {
		...
		it("should preserve verified status in query results", async () => {
			const verifiedRelation = await Effect.runPromise(
				getRelation(TEST_RELATION_1_ID).pipe(provideDeps),
			);

			const unverifiedRelation = await Effect.runPromise(
				getRelation(TEST_RELATION_2_ID).pipe(provideDeps),
			);
	...
	describe("Backlinks", () => {
		it("should return relations where toEntityId equals the entity's id", async () => {
			...
			const backlinks = await Effect.runPromise(
				getBacklinks({ id: TEST_ENTITY_2_ID }, createMockContext()).pipe(
					provideDeps,
				),
			);
		...
		it("should return multiple backlinks when entity is referenced multiple times", async () => {
			...
			const backlinks = await Effect.runPromise(
				getBacklinks({ id: TEST_ENTITY_3_ID }, createMockContext()).pipe(
					provideDeps,
				),
			);
		...
		it("should return empty array when entity has no backlinks", async () => {
			...
			const result = await Effect.runPromise(
				getBacklinks(newEntityId).pipe(provideDeps),
			);
			...
			await Effect.runPromise(
				Effect.gen(function* () {
					...
					yield* storage.use(async (client) => {
						await client
							.delete(entities)
							.where(inArray(entities.id, [newEntityId]));
					});
				}).pipe(provideDeps),
			);
		});
		...
		it("should filter backlinks by spaceId when provided", async () => {
			...
			const backlinks = await Effect.runPromise(
				getBacklinks(
					{ id: TEST_ENTITY_2_ID, spaceId: TEST_SPACE_ID },
					createMockContext(),
				).pipe(provideDeps),
			);
		...
		it("should return backlinks with all expected fields", async () => {
			const result = await Effect.runPromise(
				getBacklinks(TEST_ENTITY_2_ID).pipe(provideDeps),
			);
