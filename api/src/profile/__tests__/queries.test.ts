...
describe("GET /profile/address/:address", () => {
	it("should return profile for valid address", async () => {
		...
		const res = await app.request("/profile/address/0xab28066d9a7ddFF52B67dF699592BA7060e0d3b9")

		expect(res.status).toBe(200)
		const body = await res.json()
		expect(body).toEqual({
			spaceId: "X7WMytqfmkJaMaZ4Zh9v93",
			name: "Test User",
	...
	it("should return default profile when address has no space", async () => {
		...
		const res = await app.request("/profile/address/0x1234567890123456789012345678901234567890")
	...
	it("should normalize address to lowercase", async () => {
		...
		await app.request("/profile/address/0xAB28066D9A7DDFF52B67DF699592BA7060E0D3B9")
		...
		expect(sqlString.toLowerCase()).toContain("0xab28066d9a7ddff52b67df699592ba7060e0d3b9")
	})
})
...
describe("GET /profile/space/:spaceId", () => {
	...
	it("should return default profile when space not found", async () => {
		...
		expect(body).toEqual({
			...
			name: null,
			avatarUrl: null,
			address: "X7WMytqfmkJaMaZ4Zh9v93",
		})
	})
