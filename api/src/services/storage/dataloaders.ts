...
interface BatchingShape {
	...
	loadEntityName: (
		id: string,
	) => Effect.Effect<string | null, BatchingError, never>;
	...
	loadEntityValues: (
		entityId: string,
		spaceId?: string | null,
	) => Effect.Effect<any[], BatchingError, never>;
	loadEntityRelations: (
		entityId: string,
		spaceId?: string | null,
	) => Effect.Effect<any[], BatchingError, never>;
...
export class Batching extends Context.Tag("Batching")<
	Batching,
	BatchingShape
>() {}
			...
			const result = await Effect.runPromise(
				storage.use(async (client) => {
					...
					const allBacklinks = await client.query.relations.findMany({
						where: (relations, { inArray }) =>
							inArray(relations.toEntityId, entityIds),
					});
		...
		loadEntityDescription: (id: string) =>
			Effect.tryPromise({
				try: () => entityDescriptionsLoader.load(id),
					...
					new BatchingError({
						cause: error,
						message: `Failed to batch load entity description ${id}: ${String(
							error,
						)}`,
					}),
			}),
		...
		loadEntityValues: (entityId: string, spaceId?: string | null) =>
			Effect.tryPromise({
				try: () => entityValuesLoader.load({ entityId, spaceId }),
					...
					new BatchingError({
						cause: error,
						message: `Failed to batch load entity values ${entityId}: ${String(
							error,
						)}`,
					}),
			}),
		...
		loadEntityRelations: (entityId: string, spaceId?: string | null) =>
			Effect.tryPromise({
				try: () => entityRelationsLoader.load({ entityId, spaceId }),
					...
					new BatchingError({
						cause: error,
						message: `Failed to batch load entity relations ${entityId}: ${String(
							error,
						)}`,
					}),
			}),
		...
		loadEntityBacklinks: (entityId: string, spaceId?: string | null) =>
			Effect.tryPromise({
				try: () => entityBacklinksLoader.load({ entityId, spaceId }),
					...
					new BatchingError({
						cause: error,
						message: `Failed to batch load entity backlinks ${entityId}: ${String(
							error,
						)}`,
					}),
			}),
		...
		loadProperty: (propertyId: string) =>
			Effect.tryPromise({
				try: () => propertiesLoader.load(propertyId),
					...
					new BatchingError({
						cause: error,
						message: `Failed to batch load property ${propertyId}: ${String(
							error,
						)}`,
					}),
			}),
