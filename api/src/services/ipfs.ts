...
function validateCid(cid: string) {
  return Effect.gen(function* () {
    ...
    if (!cid.startsWith("ipfs://")) {
      yield* Effect.fail(
        new CidValidateError(`CID ${cid} does not start with ipfs://`)
      );
    }
...
export function uploadEdit(file: File) {
  const run = Effect.gen(function* () {
    ...
    const hash = yield* upload(formData, config.ipfsGatewayWrite).pipe(
      Effect.withSpan("ipfs.uploadEdit.upload")
    );
  ...
  return Effect.retry(run, {
    schedule: Schedule.exponential("100 millis").pipe(
      ...
      Schedule.tapInput(() =>
        Effect.succeed(console.log("[IPFS][upload] Retrying"))
      ),
...
export function uploadFile(file: File) {
  const run = Effect.gen(function* () {
    ...
    yield* Effect.logInfo(
      "[IPFS][upload-gateway] Uploading content..."
    );
    const hash = yield* upload(
      formData,
      config.ipfsGatewayWrite
    ).pipe(Effect.withSpan("ipfs.uploadFile.upload"));
    yield* Effect.logInfo(
      "[IPFS][upload-gateway] Uploaded to IPFS successfully"
    );
  ...
  return Effect.retry(run, {
    schedule: Schedule.exponential("100 millis").pipe(
      ...
      Schedule.tapInput(() =>
        Effect.succeed(
          console.log("[IPFS][upload-gateway] Retrying")
        )
      ),
      Schedule.whileOutput(Duration.lessThanOrEqualTo(Duration.seconds(30)))
    ),
...
export function upload(formData: FormData, url: string) {
  return Effect.gen(function* () {
    yield* Effect.logInfo("[IPFS] Posting IPFS gateway content");
    const config = yield* Environment;
    ...
    const response = yield* Effect.tryPromise({
      try: () =>
        ...
        new IpfsUploadError(`IPFS gateway upload failed: ${error}`),
    ...
    const responseJson = yield* Effect.tryPromise({
      try: () => response.json(),
        ...
        new IpfsParseResponseError(
          `Could not parse IPFS gateway JSON response: ${error}`
        ),
    });
    ...
    if (responseJson.error) {
      yield* Effect.fail(
        new IpfsUploadError(
          `IPFS gateway upload failed: ${responseJson.error}`
        )
      );
    }
    ...
    if (!responseJson.data || !responseJson.data.cid) {
      yield* Effect.fail(
        new IpfsUploadError(
          `IPFS gateway upload failed: No data.cid in response`
        )
      );
    }
