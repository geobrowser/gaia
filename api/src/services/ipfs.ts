...
export function upload(formData: FormData, url: string) {
	return Effect.gen(function* () {
		...
		if (responseJson.error) {
			yield* Effect.fail(new IpfsUploadError(`IPFS upload failed: ${responseJson.error}`))
		}

		if (!responseJson.data || !responseJson.data.cid) {
			yield* Effect.fail(new IpfsUploadError(`IPFS upload failed: No data.cid in response`))
		}
