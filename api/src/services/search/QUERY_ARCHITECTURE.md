...
6. **+ Score Fields** — additive boost from score fields via `function_score` (1.3× multiplier)
...
|----------|-------|-------|
| `SCORE_BOOST` | 1.3 | `field_value_factor` multiplier for all score fields |
...
- **Score field flexibility**: Score fields use `float` type instead of `rank_feature` to support zero and negative values from z-score normalization. Boosting is done via `function_score` with `field_value_factor`.
