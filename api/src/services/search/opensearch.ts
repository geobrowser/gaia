...
export class OpenSearchClient implements SearchClient {
	...
	async search(query: SearchQuery): Promise<SearchResponse> {
		...
		const results: SearchResult[] = hits.map((hit) => {
			...
			return {
				...
				spaceId: uuidToBase58(hit._source.space_id as string),
	...
	buildSearchBody(query: SearchQuery): object {
		...
		const uuidFromQuery = tryFromBase58(trimmedQuery)
		if (uuidFromQuery) {
			return this.buildUuidQuery(uuidFromQuery, query.scope, query.space_id, query.type_ids, includeDeleted)
		}
