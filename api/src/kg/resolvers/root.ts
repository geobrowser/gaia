...
} from "~/src/services/storage/dataloaders";
...
export const entityRelations = (
	args: QueryEntityArgs & { spaceId?: string | null },
) => {
	return Effect.runPromise(
...
export const propertyRelationValueTypes = (
	args: QueryEntityArgs & { dataType: DataType },
) => {
...
export const types = (args: QueryTypesArgs) => {
	return Effect.runPromise(
		TypeResolvers.getTypes(args).pipe(
			Effect.withSpan("getTypes"),
			Effect.annotateSpans({ ...args }),
			provideDeps,
		),
	);
...
export const search = (args: QuerySearchArgs) => {
	return Effect.runPromise(
		SearchResolvers.search(args).pipe(
			Effect.withSpan("search"),
			Effect.annotateSpans({ ...args }),
			provideDeps,
		),
	);
...
export const spaces = (args: QuerySpacesArgs) => {
	return Effect.runPromise(
		SpaceResolvers.getSpaces(args).pipe(
			Effect.withSpan("getSpaces"),
			Effect.annotateSpans({ ...args }),
			provideDeps,
		),
...
export const space = (id: string) => {
	return Effect.runPromise(
		SpaceResolvers.getSpace(id).pipe(
			Effect.withSpan("getSpace"),
...
export const meta = () => {
	return Effect.runPromise(
		MetaResolvers.getMeta().pipe(Effect.withSpan("Query.meta"), provideDeps),
	);
};
