...
export function getEntity(id: string, context: GraphQLContext) {
	return Effect.gen(function* () {
		const entity = yield* Effect.tryPromise({
			...
			Effect.annotateSpans({ entityId: id }),
			Effect.withSpan("getEntity.loadEntity"),
		);
...
export function getEntityName(id: string, context: GraphQLContext) {
	return Effect.gen(function* () {
		const name = yield* Effect.tryPromise({
			...
			Effect.annotateSpans({ entityId: id }),
			Effect.withSpan("getEntityName.loadEntityName"),
		);
...
export function getEntityDescription(id: string, context: GraphQLContext) {
	return Effect.gen(function* () {
		const description = Effect.tryPromise({
			...
			Effect.annotateSpans({ entityId: id }),
			Effect.withSpan("loadEntityDescription"),
		);
...
export function getValues(
	...
	return Effect.gen(function* () {
		const values = yield* Effect.tryPromise({
			try: () =>
				context.entityValuesLoader.load({ entityId: id, spaceId, filter }),
			catch: (error) =>
				new BatchingError({
					cause: error,
			...
			Effect.withSpan("getValues.loadEntityValues"),
		);
...
export function getRelations(
	...
	return Effect.gen(function* () {
		const relations = yield* Effect.tryPromise({
			try: () =>
				context.entityRelationsLoader.load({ entityId: id, spaceId, filter }),
			catch: (error) =>
				new BatchingError({
					cause: error,
...
export function getBacklinks(
	...
	return Effect.gen(function* () {
		const backlinks = yield* Effect.tryPromise({
			try: () =>
				context.entityBacklinksLoader.load({ entityId: id, spaceId, filter }),
			catch: (error) =>
				new BatchingError({
					cause: error,
			...
			Effect.annotateSpans({ entityId: id, spaceId, filter }),
			Effect.withSpan("loadEntityBacklinks"),
		);
...
export function getEntityTypes(id: string, context: GraphQLContext) {
	return Effect.gen(function* () {
		const relations = yield* Effect.tryPromise({
			...
			Effect.annotateSpans({ entityId: id }),
			Effect.withSpan("getEntityTypes.loadEntityRelations"),
		);

		// Filter for type relations and load the target entities
		const typeRelations = relations.filter(
			(relation) => relation.typeId === SystemIds.TYPES_PROPERTY,
		);
		...
		const typeEntities = yield* Effect.forEach(
			...
			(relation) =>
				Effect.tryPromise({
					...
					Effect.annotateSpans({ entityId: id }),
					Effect.withSpan("getEntityTypes.loadEntity"),
				),
	...
	return Effect.gen(function* () {
		...
		const [values, relations] = yield* Effect.all([
			Effect.tryPromise({
				...
				Effect.annotateSpans({ entityId: id }),
				Effect.withSpan("getSpaces.loadEntityValues"),
			),
			Effect.tryPromise({
				try: () => context.entityRelationsLoader.load({ entityId: id }),
				catch: (error) =>
				...
				Effect.annotateSpans({ entityId: id }),
				Effect.withSpan("getSpaces.loadEntityRelations"),
			),
		]);
...
export function getBlocks(entityId: string, context: GraphQLContext) {
	return Effect.gen(function* () {
		...
		return blockEntities
			.filter(
				(blockData): blockData is NonNullable<typeof blockData> =>
					blockData !== null,
			)
			.map((blockData) => {
				const { entity, relations: fromRelations, values } = blockData;
				const blockTypeId =
					fromRelations.find((r) => r.typeId === SystemIds.TYPES_PROPERTY)
						?.toEntityId ?? null;
				...
				if (blockTypeId === SystemIds.TEXT_BLOCK) {
					type = BlockType.Text;
					value =
						values.find((v) => v.propertyId === SystemIds.MARKDOWN_CONTENT)
							?.value ?? null;
				} else if (blockTypeId === SystemIds.IMAGE_TYPE) {
					type = BlockType.Image;
					value =
						values.find((v) => v.propertyId === SystemIds.IMAGE_URL_PROPERTY)
							?.value ?? null;
				} else if (blockTypeId === SystemIds.DATA_BLOCK) {
					type = BlockType.Data;
					value =
						values.find((v) => v.propertyId === SystemIds.FILTER)?.value ??
						null;
