...
describe("GeoPointScalar", () => {
  ...
  it("serializes values as-is", () => {
    expect(GeoPointScalar.serialize("40.7128,-74.0060")).toBe(
      "40.7128,-74.0060",
    );
    expect(GeoPointScalar.serialize("40.7128,-74.0060,10.5")).toBe(
      "40.7128,-74.0060,10.5",
    );
  });

  it("parses string values", () => {
    expect(GeoPointScalar.parseValue("40.7128,-74.0060")).toBe(
      "40.7128,-74.0060",
    );
  });

  it("rejects non-string values", () => {
    expect(() => GeoPointScalar.parseValue(123)).toThrow(
  ...
  it("parses string literals", () => {
    expect(
      GeoPointScalar.parseLiteral({
        kind: Kind.STRING,
        value: "40.7128,-74.0060",
      }),
    ).toBe("40.7128,-74.0060");
  });

  it("rejects non-string literals", () => {
    expect(() =>
      GeoPointScalar.parseLiteral({ kind: Kind.INT, value: "123" } as any),
    ).toThrow("GeoPoint must be a string");
  });
});
...
describe("GeoRectScalar", () => {
  it("has descriptive name and description", () => {
    ...
    expect(GeoRectScalar.description).toContain(
      "min_lat,min_lon,max_lat,max_lon",
    );
  });

  it("serializes values as-is", () => {
    expect(GeoRectScalar.serialize("24.5,-125.0,49.4,-66.9")).toBe(
      "24.5,-125.0,49.4,-66.9",
    );
  });

  it("parses string values", () => {
    expect(GeoRectScalar.parseValue("24.5,-125.0,49.4,-66.9")).toBe(
      "24.5,-125.0,49.4,-66.9",
    );
  });

  it("rejects non-string values", () => {
    expect(() => GeoRectScalar.parseValue(123)).toThrow(
      "GeoRect must be a string",
    );
    expect(() =>
      GeoRectScalar.parseValue({
        minLat: 24.5,
        minLon: -125,
        maxLat: 49.4,
        maxLon: -66.9,
  ...
  it("parses string literals", () => {
    expect(
      GeoRectScalar.parseLiteral({
        kind: Kind.STRING,
        value: "24.5,-125.0,49.4,-66.9",
      }),
  ...
  it("rejects non-string literals", () => {
    expect(() =>
      GeoRectScalar.parseLiteral({ kind: Kind.INT, value: "123" } as any),
    ).toThrow("GeoRect must be a string");
  });
});
...
describe("DateStringScalar", () => {
  ...
  it("rejects non-string values", () => {
    expect(() => DateStringScalar.parseValue(20240115)).toThrow(
...
describe("DateTimeStringScalar", () => {
  ...
  it("parses and serializes string values", () => {
    expect(DateTimeStringScalar.parseValue("2024-01-15T14:30:00")).toBe(
      "2024-01-15T14:30:00",
    );
...
describe("TimeStringScalar", () => {
  ...
  it("rejects non-string values", () => {
    expect(() => TimeStringScalar.parseValue(143000)).toThrow(
...
describe("BytesScalar", () => {
  ...
  it("rejects non-string values", () => {
    expect(() => BytesScalar.parseValue(Buffer.from("test"))).toThrow(
...
describe("DecimalStringScalar", () => {
  it("has descriptive name and description", () => {
    expect(DecimalStringScalar.name).toBe("DecimalString");
    expect(DecimalStringScalar.description).toContain("Arbitrary-precision");
  });

  it("serializes string values as-is", () => {
    expect(DecimalStringScalar.serialize("123.456789")).toBe("123.456789");
    expect(DecimalStringScalar.serialize("0.001")).toBe("0.001");
  });

  it("coerces numeric values to string", () => {
    expect(DecimalStringScalar.serialize(123.45)).toBe("123.45");
    expect(DecimalStringScalar.serialize(0)).toBe("0");
  });

  it("parses string values", () => {
    expect(DecimalStringScalar.parseValue("123.456789")).toBe("123.456789");
  });

  it("rejects non-string values", () => {
    expect(() => DecimalStringScalar.parseValue(123.45)).toThrow(
      "DecimalString must be a string",
    );
  });

  it("parses string literals", () => {
    expect(
      DecimalStringScalar.parseLiteral({
        kind: Kind.STRING,
        value: "123.456789",
      }),
    ).toBe("123.456789");
  ...
  it("rejects non-string literals", () => {
    ...
    ).toThrow("DecimalString must be a string");
  });
});
...
describe("ValueScalarsPlugin schema integration", () => {
  it("should expose all custom scalar types", async () => {
    const result = await executeGraphQL(`
			query IntrospectScalars {
				...
				decimalString: __type(name: "DecimalString") { name kind }
			}
		`);
    ...
    expect(result.data.dateString).toEqual({
      name: "DateString",
      kind: "SCALAR",
    });
    expect(result.data.dateTimeString).toEqual({
      name: "DateTimeString",
      kind: "SCALAR",
    });
    expect(result.data.timeString).toEqual({
      name: "TimeString",
      kind: "SCALAR",
    });
    expect(result.data.bytes).toEqual({ name: "Bytes", kind: "SCALAR" });
    expect(result.data.languageTag).toEqual({
      name: "LanguageTag",
      kind: "SCALAR",
    });
    expect(result.data.decimalString).toEqual({
      name: "DecimalString",
      kind: "SCALAR",
    });
  });
  ...
  it("should use custom scalars for Value type fields", async () => {
    ...
    const getFieldType = (name: string) => {
      const field = result.data.__type?.fields?.find(
    ...
    expect(getFieldType("decimal")).toBe("DecimalString");
  });
