...
// --------------------------------------------------------------------------
// Fast-path auto-exec reads flat_support_threshold (not the legacy threshold).
// --------------------------------------------------------------------------
#[tokio::test]
#[ignore]
async fn test_process_tally_queue_auto_exec_uses_flat_threshold() {
    ...
    // Fast-path: flat_support_threshold = 1. Legacy `threshold` set to 99 to
    // prove the auto-exec query reads the V2 column and not the legacy one.
    let mut v1 = version_fast(1, Some(9_999_999_999));
    v1.threshold = 99;
    seed_proposal_v1(&storage, &pool, proposal_id, space_id, proposer_id, &v1).await;
    ...
    let executed_at: Option<i64> = sqlx::query_as::<_, (Option<i64>,)>(
        r#"SELECT pv.executed_at
           FROM proposals p
           JOIN proposal_versions pv
             ON pv.proposal_id = p.id AND pv.proposal_version = p.current_version
           WHERE p.id = $1"#,
    ...
    assert!(
        executed_at.is_some(),
        "fast-path auto-exec must fire when yes_count >= flat_support_threshold"
    );
...
// --------------------------------------------------------------------------
// Fast-path auto-exec must NOT fire when the latest vote is past execute_by.
// --------------------------------------------------------------------------
#[tokio::test]
#[ignore]
async fn test_process_tally_queue_respects_execute_by_deadline() {
    ...
    // execute_by at t=1_700_000_050, vote at t=1_700_000_100 — past deadline.
    let v1 = version_fast(1, Some(1_700_000_050));
    seed_proposal_v1(&storage, &pool, proposal_id, space_id, proposer_id, &v1).await;
