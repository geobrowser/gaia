...
impl ConsumeActionsStream for SubstreamsStreamProvider {
    async fn stream_events(
        ...
        let package = read_package(&self.package_file, self.params.clone())
            .await
            .map_err(|e| ConsumerError::ReadingPackage(e.to_string()))?;
        let block_range = read_block_range(&package, &self.module_name, self.block_range.clone())
            .map_err(|e| ConsumerError::ReadingBlockRange(e.to_string()))?;

        let endpoint = Arc::new(
            SubstreamsEndpoint::new(&self.endpoint_url, self.token.clone())
                .await
                .map_err(|e| ConsumerError::ReadingEndpoint(e.to_string()))?,
        );
            ...
            match stream.next().await {
                ...
                Some(Ok(BlockResponse::New(data))) => {
                    let actions = self
                        ...
                        .map_err(|e| ConsumerError::ProcessingBlockScopedData(e.to_string()))?;
                    sender
                ...
                Some(Err(err)) => {
                    ...
                    println!("{:?}", err);
                    sender
                        .send(StreamMessage::Error(ConsumerError::StreamingError(
                            err.to_string(),
                        )))
                        .await
                        .map_err(|e| ConsumerError::ChannelSend(e.to_string()))?;
    ...
    fn try_from(action: &Action) -> Result<Self, Self::Error> {
        Ok(ActionRaw {
            ...
            action_type: match action.action_type {
                0 => ActionType::Vote,
                _ => {
                    return Err(ConsumerError::InvalidActionType(format!(
                        "action_type: {}",
                        action.action_type
                    )));
                }
