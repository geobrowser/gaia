...
mod tests {
    ...
    async fn test_mixed_type_relation_operations() {
        ...
        // 1 RemoveTypeRelationById + 2 bulk operations (AddTypeRelation and Index)
        assert_eq!(provider.get_operation_count(), 3);

        // Verify the bulk operations are in order
        let ops = provider.get_operation_order();
        assert_eq!(ops.len(), 2); // Only bulk ops tracked here
        ...
        // Second should be the regular index (Update with no add_type_relation)
        assert!(matches!(
            &ops[1],
    ...
    async fn test_type_relation_operations_preserve_order() {
        ...
        // The AddTypeRelation should be the bulk operation
        assert!(
            matches!(&ops[0], TrackedOperation::Update { add_type_relation: Some(rel), .. } if rel.entity_to_id == entity_to_id.to_string()),
            "Operation should be AddTypeRelation (via Update), got: {:?}",
            ops[0]
        );
    }
