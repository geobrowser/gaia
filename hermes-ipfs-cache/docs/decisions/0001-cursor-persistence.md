...
The hermes-relay `Sink` trait calls `persist_cursor` after each `process_block_scoped_data` returns. We could persist immediately and rely on downstream consumers (`hermes-pipeline`) to retry on cache miss.
